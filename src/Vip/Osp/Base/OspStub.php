...
namespace lccong\EasyTBK\Vip\Osp\Base;

//use Thrift\Transport\TFramedTransport;
//use Thrift\Type\TMessageType;
//use Thrift\Transport\TSocket;
//use Thrift\Transport\TBufferedTransport;
use lccong\EasyTBK\Vip\Osp\Context\InvocationContextFactory;
...
class OspStub
{
    ...
    {
        \lccong\EasyTBK\Vip\Osp\Log\Logger::info("[OspStub][InitInvocation][Service-Version-Method][" . $this->serviceName . "-" . $this->version . "-" . $method . "]");
        ...
        , $timestamp, $version, $request, $appSecret)
    {
        ...
        return \lccong\EasyTBK\Vip\Osp\Util\HmacUtil::hmac($sign, $appSecret);
    }
    ...
    {
        if (!$restAddr) throw new \lccong\EasyTBK\Vip\Osp\Exception\OspException("The openapi appURL is null, please set the appURL value");
    ...
    {
        $ctx = InvocationContextFactory::getInstance();
        $oprot = new \lccong\EasyTBK\Vip\Osp\Protocol\JSONProtocol(new \lccong\EasyTBK\Vip\Osp\Buffer\MemoryBuffer());
        $args->write($oprot);
        ...
        $client = new \lccong\EasyTBK\Vip\Osp\Http\HttpClient($scheme, $host, $port);
        $request = array();

        $appKey = $ctx->getAppKey();
        if ($appKey == null) throw new \lccong\EasyTBK\Vip\Osp\Exception\OspException("The openapi appKey is null, please set the appKey value");

        $appSecret = $ctx->getAppSecret();
        if ($appSecret == null) throw new \lccong\EasyTBK\Vip\Osp\Exception\OspException("The openapi appSecret is null, please set the appSecret value");
        ...
        $timestamp = round(\lccong\EasyTBK\Vip\Osp\Util\TimeUtil::currentTimeMillis() / 1000);
        ...
        try {
            ...
            throw new \lccong\EasyTBK\Vip\Osp\Exception\OspException(\Osp\Util\MessageUtil::getInvocationMsg($ctx, $e->getMessage()), $e);
        }
    ...
    public function receive_base($result)
    {
        ...
        if (\lccong\EasyTBK\Vip\Osp\Base\OspStub::$RETURN_NULL != $response) {
            $iprot = new \lccong\EasyTBK\Vip\Osp\Protocol\JSONProtocol(new \lccong\EasyTBK\Vip\Osp\Buffer\MemoryBuffer());
            ...
            if (!"0" == $returnCode) {
                ...
                throw new \lccong\EasyTBK\Vip\Osp\Exception\OspException($returnMessage, $returnCode);
            }
